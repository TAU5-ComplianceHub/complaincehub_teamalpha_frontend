...
const FTS_VERSION_HISTORY_CONFIG = {
    template: {
        apiBase: `${process.env.REACT_APP_URL}/api/ftsDrafts/templates`,
        editorRoute: (draftId) => `/FrontendDMS/ftsCreateTemplate/template/${draftId}`,
        previewRoute: (draftId, version) => `/FrontendDMS/ftsDraftVersionPreview/template/${draftId}/${version}`,
        icon: "/ibra2.svg",
    },
};

const FTSVersionHistoryPage = () => {
    ...
    const pageConfig = FTS_VERSION_HISTORY_CONFIG[normalizedType];

    const loadVersions = async () => {
        setIsLoading(true);

        if (!pageConfig) {
            toast.error("Unsupported field template draft type");
            setIsLoading(false);
            return;
        }
        ...
        try {
            ...
            console.error("Failed to load FTS version history:", error);
    ...
    const openPreview = (version) => {
        ...
        navigate(pageConfig.previewRoute(id, version));
    };
    ...
    const restoreVersion = async (item) => {
        ...
        try {
            ...
            console.error("Failed to restore FTS version:", error);
...
export default FTSVersionHistoryPage;
