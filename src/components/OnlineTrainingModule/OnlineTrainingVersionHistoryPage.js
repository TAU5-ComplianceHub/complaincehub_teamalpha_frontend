...
const API_BASE = `${process.env.REACT_APP_URL}/api/onlineTrainingCourses`;
const EDITOR_ROUTE = (draftId) => `/FrontendDMS/onlineCreateCourse/${draftId}`;
const PREVIEW_ROUTE = (draftId, version) => `/FrontendDMS/tmsDraftVersionPreview/onlineTraining/${draftId}/${version}`;

const OnlineTrainingVersionHistoryPage = () => {
    ...
    const loadVersions = async () => {
        ...
        try {
            ...
            console.error("Failed to load online training version history:", error);
    ...
    const openPreview = (version) => {
        navigate(PREVIEW_ROUTE(id, version));
    };
    ...
    const restoreVersion = async (item) => {
        ...
        try {
            const response = await fetch(
                `${API_BASE}/versions/${id}/${item.version}/restore`,
                {
                    method: "POST",
                    headers: {
                        "Authorization": `Bearer ${localStorage.getItem("token")}`,
            ...
            console.error("Failed to restore online training version:", error);
...
export default OnlineTrainingVersionHistoryPage;
