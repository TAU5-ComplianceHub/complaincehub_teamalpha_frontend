...
const SignedOffSpecial = () => {
    ...
    const startReview = async () => {
        ...
        let route = `${process.env.REACT_APP_URL}/api/docCreate/reviewSOSpecialIns/${fileToReview}`
    ...
    const fetchFiles = async () => {
        const route = isSystemAdmin
            ? `${process.env.REACT_APP_URL}/api/fileGenDocs/signedOffSpecial/${userID}?isAdmin=true`
            : `${process.env.REACT_APP_URL}/api/fileGenDocs/signedOffSpecial/${userID}`;
    ...
    const allColumns = [
        ...
        { id: "name", title: "Document Name", thClass: "gen-th ibraGenFN", tdClass: "gen-point", onCellClick: (f) => setHoveredFileId(hoveredFileId === f._id ? null : f._id), td: (f) => (<div className="popup-anchor"><span>{(f.formData.title)}</span>{(hoveredFileId === f._id) && (<PopupMenuSignedOffFiles file={f} typeDoc={"special"} risk={false} isOpen={true} openDownloadModal={downloadFile} setHoveredFileId={setHoveredFileId} id={f._id} review={openReviewPopup} />)}</div>) },
