...
const GeneratedStandardsInfo = () => {
    ...
    const fetchFiles = async () => {
        const route = isSystemAdmin
            ? `${process.env.REACT_APP_URL}/api/fileGenDocs/standard/${userID}?isAdmin=true`
            : `${process.env.REACT_APP_URL}/api/fileGenDocs/standard/${userID}`;
    ...
    const allColumns = [
        ...
        { id: "name", title: "Document Name", thClass: "gen-th ibraGenFN", tdClass: "gen-point", onCellClick: (f) => setHoveredFileId(hoveredFileId === f._id ? null : f._id), td: (f) => (<div className="popup-anchor"><span>{(f.formData.title)}</span>{(hoveredFileId === f._id) && (<PopupMenuPubFiles file={f} typeDoc={"standard"} risk={false} isOpen={true} openDownloadModal={downloadFile} setHoveredFileId={setHoveredFileId} id={f._id} openProcedurePopup={openPDFUpload} type={"standard"} />)}</div>) },
