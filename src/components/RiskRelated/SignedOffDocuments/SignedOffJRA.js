...
const SignedOffJRA = () => {
    ...
    const startReview = async () => {
        ...
        let route = `${process.env.REACT_APP_URL}/api/t/reviewSOJRA/${fileToReview}`
    ...
    const fetchFiles = async () => {
        const route = isSystemAdmin
            ? `${process.env.REACT_APP_URL}/api/fileGenDocs/signedOffJRA/${userID}?isAdmin=true`
            : `${process.env.REACT_APP_URL}/api/fileGenDocs/signedOffJRA/${userID}`;
    ...
    const allColumns = [
        ...
        { id: "name", title: "Document Name", thClass: "gen-th ibraGenFN", tdClass: "gen-point", onCellClick: (f) => setHoveredFileId(hoveredFileId === f._id ? null : f._id), td: (f) => (<div className="popup-anchor"><span>{(f.formData.title)}</span>{(hoveredFileId === f._id) && (<RiskPopupMenuSignedOffFiles file={f} typeDoc={"jra"} risk={false} isOpen={true} openDownloadModal={downloadFile} setHoveredFileId={setHoveredFileId} id={f._id} review={openReviewPopup} />)}</div>) },
