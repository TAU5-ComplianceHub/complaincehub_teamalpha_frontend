...
const InductionCreationPage = ({ versionPreview = false }) => {
  ...
  const handleRenameDraft = async (newTitle) => {
    ...
    try {
      const response = await fetch(`${process.env.REACT_APP_URL}/api/visitorDrafts/rename/${activeId}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${localStorage.getItem("token")}`,
  ...
  const handleSaveVersion = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.REACT_APP_URL}/api/visitorDrafts/versions/${activeDraftId}`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`
      ...
      console.error("Error saving induction version:", error);
      toast.dismiss();
      toast.clearWaitingQueue();
  ...
  const rejectDraft = async (message) => {
    ...
    try {
      const response = await fetch(`${process.env.REACT_APP_URL}/api/visitorDrafts/reject-draft`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          "Authorization": `Bearer ${localStorage.getItem("token")}`
  ...
  const loadData = async (loadID) => {
    try {
      ...
      const loadUrl = versionPreview && versionNumber
        ? `${process.env.REACT_APP_URL}/api/visitorDrafts/versions/${loadID}/${versionNumber}`
  ...
  const resolveWithdrawnDraft = async (reshare, options = {}) => {
    ...
    try {
      const response = await fetch(`${process.env.REACT_APP_URL}/api/visitorDrafts/resolveWithdrawn/${loadedIDRef.current}`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          "Authorization": `Bearer ${localStorage.getItem("token")}`
  ...
  const resolveRejectedDraft = async () => {
    try {
      const response = await fetch(`${process.env.REACT_APP_URL}/api/visitorDrafts/resolveRejected/${loadedIDRef.current}`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          "Authorization": `Bearer ${localStorage.getItem("token")}`
  ...
  const handleBack = () => {
    const navigateBack = () => versionPreview
      ? navigate(-1)
      : navigate("/FrontendDMS/visitorInductionHome");
      ...
      <div className="main-box-create">
        ...
        {isRenamePopupOpen && (
          <RenameDraftPopup
            ...
            siblingDraftsRoute={`${process.env.REACT_APP_URL}/api/visitorDrafts/getDrafts/${userID}`}
