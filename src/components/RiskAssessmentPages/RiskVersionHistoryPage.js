...
const RISK_VERSION_HISTORY_CONFIG = {
    ibra: {
        apiBase: `${process.env.REACT_APP_URL}/api/riskDraft/ibra`,
        editorRoute: (draftId) => `/FrontendDMS/riskIBRA/IBRA/${draftId}`,
        icon: "/ibra2.svg",
    },
    jra: {
        apiBase: `${process.env.REACT_APP_URL}/api/riskDraft/jra`,
        editorRoute: (draftId) => `/FrontendDMS/riskJRA/JRA/${draftId}`,
        icon: "/jra2.svg",
    },
    blra: {
        apiBase: `${process.env.REACT_APP_URL}/api/riskDraft/blra`,
        editorRoute: (draftId) => `/FrontendDMS/riskBLRA/BLRA/${draftId}`,
        icon: "/blra2.svg",
    },
};

const RiskVersionHistoryPage = () => {
    ...
    const pageConfig = RISK_VERSION_HISTORY_CONFIG[normalizedType];

    const loadVersions = async () => {
        setIsLoading(true);

        if (!pageConfig) {
            toast.error("Unsupported risk draft type");
            setIsLoading(false);
            return;
        }
        ...
        try {
            ...
            console.error("Failed to load risk version history:", error);
    ...
    const openPreview = (version) => {
        navigate(
            `/FrontendDMS/riskVersionPreview/${encodeURIComponent(normalizedType.toUpperCase())}/${encodeURIComponent(id)}/${version}`
        );
    };

    const requestRestore = (item) => {
        if (!canRestore || restoringVersion || !pageConfig) return;
    ...
    const restoreVersion = async (item) => {
        ...
        try {
            ...
            console.error("Failed to restore risk version:", error);
...
export default RiskVersionHistoryPage;
